...
const (
	...
	envLoggingFmt = "GOLOG_LOG_FMT" // noccolor or json
...
func ConfigFromEnv() Config {
	...
	level := os.Getenv(envLogging)
	if level == "" {
		level = os.Getenv(envIPFSLogging)
	}
	if level != "" {
		var err error
		cfg.Level, err = LevelFromString(level)
		if err != nil {
...
func SetupLogging(cfg Config) {
	// Reset the config to prevent stale values being carried over
	...
	zapCfg.Level.SetLevel(zapcore.Level(cfg.Level))
...
func getLogger(name string) *zap.SugaredLogger {
	...
	if !ok {
		...
		log = zap.L().Named(name).WithOptions(zap.IncreaseLevel(levels[name])).Sugar()
		loggers[name] = log
	}
